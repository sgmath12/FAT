...
\Cref{eqn:dual} reads the gradient in $\ell_1$ because it asks for the worst direction in the ball;
the typical-direction reading of \Cref{sec:anchor} gives $\ell_2$, and that is the one we run. The
honest history is that this was not chosen between: the cell that fixed the recipe used $\ell_2$ and
every logged run since had to stay reproducible against it, so the second reading is a justification
found afterwards rather than the reason. That makes the measurement below the load-bearing part, not
the argument. The two norms are not the same allocation.
...
They nonetheless land in the same place. Training the shipped recipe with
...
identical to a hundredth and AutoAttack differs by $0.29$, in favour of the norm we ship. That a
genuinely different allocation produces the same frontier point is the strongest form the claim can
take --- the rule is insensitive to the norm at first order, and the derivation can be read as stated
without the shipped constant being a silent departure from it.
