...
This variant yields $0.00\%$ AutoAttack accuracy (\Cref{tab:controls}). With a fixed clean target,
...
whereas the angle to the nearest class centroid increases from $35.7^\circ$ to $44.9^\circ$. The
...
\begin{table}[!htbp]
  \centering \small \setlength{\tabcolsep}{4.5pt}
  \caption{Does the anchor need the teacher's feature \emph{magnitude}? Normalizing both
  representations to the unit sphere removes it and keeps only the angle, which makes the squared
  Euclidean loss $\lVert \hat\Phi_s - \hat\Phi_t \rVert_2^2 = 2 - 2\cos\angle(\Phi_s, \Phi_t)$, i.e.\
  a cosine loss up to an affine map. CIFAR-100, ResNet-18, 100 epochs; within each panel the two rows
  differ in the normalization and in nothing else. The asymmetric row normalizes the student alone, so
  the two sides live at different scales; it is a diagnostic rather than a design, and is marked as
  such.}
...
\paragraph{Feature magnitude, and what the anchor actually transfers.} The objective regresses raw
features while the geometry read in \Cref{sec:analysis} is angular, so a natural question is whether
the gain is magnitude matching rather than directional agreement. \Cref{tab:normalization} removes the
magnitude by normalizing both sides. Under the full recipe, directional matching keeps the standard
accuracy of raw regression and differs by $0.07$ AutoAttack and $0.18$ NRR; taking each design at its
own best recipe instead, raw regression reaches $62.17 / 28.86$ (NRR $39.42$) against $62.17 / 28.59$
(NRR $39.17$) for the directional variant, within $0.25$ NRR of each other, though those two cells
also differ in how they treat the classifier and the late-schedule learning rate. In the base regime
raw regression is the better of the two by $1.44$ AutoAttack, so feature magnitudes do help
optimization there without being necessary for the final numbers.
Read together with \Cref{tab:targetcomparison_values}, where logit regression against the same frozen
head lands $0.21$ NRR below the feature anchor, the conclusion is not that one metric is privileged.
What the three share is the design: the teacher's clean representation is handed to the student as a
fixed target, and no label objective is added beside it.
...
\begin{table}[!htbp]
  ...
  \caption{The stack, on both datasets and both training radii. Every cell is the anchor with the
  sensitivity-matched radius and the teacher's classifier inherited; the columns add weight averaging
  ($\kappa = 0.999$ from $20\%$ of the run) and then the AWP proxy ($\gamma = 0.005$, warmup at $10\%$
  of the run). Empty cells were not run. Two readings hold on both datasets: without AWP the shorter
  schedule is the better one, and AWP is what makes the $100$-epoch schedule pay.}
  \label{tab:stackboth}
  \begin{tabular}{lrrrrrrrrr}
    \toprule
    & \multicolumn{3}{c}{No stack} & \multicolumn{3}{c}{$+$ WA} & \multicolumn{3}{c}{$+$ WA $+$ AWP} \\
    \cmidrule(lr){2-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}
    & Clean & AA & NRR & Clean & AA & NRR & Clean & AA & NRR \\
    \midrule
    \multicolumn{10}{l}{\emph{CIFAR-100}} \\
    $8/255$, 50 ep & 64.51 & 25.88 & 36.94 & 62.90 & 27.76 & 38.52 & -- & -- & -- \\
    $8/255$, 100 ep & -- & -- & -- & 63.54 & 26.75 & 37.65 & 63.89 & 27.78 & 38.72 \\
    $8.8/255$, 50 ep & 62.91 & 26.50 & 37.29 & 61.24 & 28.30 & 38.71 & 59.74 & 28.01 & 38.14 \\
    $8.8/255$, 100 ep & 63.09 & 25.74 & 36.56 & 62.30 & 27.69 & 38.34 & 62.17 & 28.86 & 39.42 \\
    \midrule
    \multicolumn{10}{l}{\emph{CIFAR-10}} \\
    $8/255$, 50 ep & 87.09 & 47.53 & 61.50 & 85.52 & 50.10 & 63.18 & 84.48 & 50.47 & 63.19 \\
    $8/255$, 100 ep & -- & -- & -- & 86.67 & 49.20 & 62.77 & 87.22 & 51.15 & 64.48 \\
    $8.8/255$, 50 ep & 85.85 & 47.98 & 61.56 & 83.54 & 50.25 & 62.75 & -- & -- & -- \\
    $8.8/255$, 100 ep & -- & -- & -- & 85.16 & 50.15 & 63.13 & 84.96 & 51.74 & 64.31 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \caption{ARREST at our recipe, matched epoch by epoch. Both columns train at $8/255$ from the same
  $200$-epoch natural teacher with our optimizer and our stack, and differ only in the objective: ours
  is the anchor with the sensitivity-matched radius, theirs is the label loss plus the angular
  representation term with the CE attack and Noisy Replay. \emph{ARREST $+$ our radius} adds our
  allocation rule to their objective.}
  \label{tab:arrestepochs}
  \begin{tabular}{lrrrcrrr}
    \toprule
    & \multicolumn{3}{c}{CFA (ours)} & & \multicolumn{3}{c}{ARREST} \\
    \cmidrule(lr){2-4}\cmidrule(lr){6-8}
    Epochs & Clean & AA & NRR & & Clean & AA & NRR \\
    \midrule
    \multicolumn{8}{l}{\emph{CIFAR-100}} \\
    100 & 63.89 & \textbf{27.78} & \textbf{38.72} & & 64.87 & 27.08 & 38.21 \\
    150 & 64.75 & \textbf{27.98} & \textbf{39.07} & & 64.66 & 26.40 & 37.49 \\
    \midrule
    \multicolumn{8}{l}{\emph{CIFAR-10}} \\
    50  & 84.48 & 50.47 & 63.19 & & 84.49 & 50.61 & 63.30 \\
    100 & 87.22 & 51.15 & \textbf{64.48} & & 85.24 & 51.52 & 64.22 \\
    150 & 87.37 & 51.33 & \textbf{64.67} & & 85.79 & 51.84 & 64.63 \\
    200 & 87.76 & 51.44 & \textbf{64.86} & & 86.13 & 51.33 & 64.33 \\
    \midrule
    \multicolumn{8}{l}{\emph{ARREST $+$ our radius, against our row at the same radius}} \\
    100, $8.8/255$, C100 & 62.17 & \textbf{28.86} & \textbf{39.42} & & 64.08 & 27.53 & 38.51 \\
    100, $8.8/255$, C10  & 84.96 & 51.74 & 64.31 & & 85.93 & 51.76 & \textbf{64.60} \\
    \bottomrule
  \end{tabular}
\end{table}
...
asserted. \Cref{tab:gradmask} runs the checks of \citet{athalye2018obfuscated} on the full test set. The model
is the seed-1 repeat of the shipped CIFAR-100 configuration ($62.00$ clean, $28.74$ AutoAttack against
the seed-0 run's $62.17 / 28.86$ reported elsewhere), which is the checkpoint that survives on disk;
none of the checks below is a property that a seed changes.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{llr}
    ...
    APGD-CE $\rightarrow$ APGD-T & what this paper reports & 28.73 \\
...
Four things follow. The black-box attack is the weakest of the four, by $5.99$ points against the
strongest white-box one, where masked gradients produce the opposite ordering. Adding FAB-T and Square
to the cascade changes the reported number by $0.00$: neither attack breaks a single one of the $2873$
points left standing by APGD-CE and APGD-T, which is why the two-attack figure is what we report
throughout. Accuracy falls monotonically with the radius and reaches exactly zero at $64/255$, so
there is no radius at which the attack stops working, and it falls monotonically with the number of
steps and flattens by $50$ rather than oscillating.
...
the name implies. Temperature also stops being a small effect once the stack is present: the two differ by
$0.48$ without it, and $\tau = 16$ reaches only $42.45 / 20.10$ with it, against $\tau = 4$'s
$56.97 / 26.96$.
...
internally consistent even though its absolute values differ from the corrected ones by a few
hundredths.
