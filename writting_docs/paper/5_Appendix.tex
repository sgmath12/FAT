...
The geometry and displacement diagnostics in \Cref{sec:empirical_geometry} use the earlier
...
\begin{table}[!htbp]
  ...
  our optimizer and schedule, without weight averaging or AWP.}
...
\begin{table}[!htbp]
  \centering \small \setlength{\tabcolsep}{3.4pt}
  \caption{Epoch-matched CFA and full-stack ARREST with the same 200-epoch natural teacher,
  optimizer, WA, and AWP. The first two panels use a mean training radius of $8/255$;
  the final panel also gives ARREST our radius allocation at $8.8/255$.
  $\Delta$AA is CFA minus ARREST, in percentage points.}
  \label{tab:arrestepochs}
  \begin{tabular}{lrrrcrrrr}
    \toprule
    & \multicolumn{3}{c}{CFA (ours)} & & \multicolumn{3}{c}{ARREST + WA + AWP} & \\
    \cmidrule(lr){2-4}\cmidrule(lr){6-8}
    Epochs & Clean & AA & NRR & & Clean & AA & NRR & $\Delta$AA \\
    \midrule
    \multicolumn{9}{l}{\emph{CIFAR-100}} \\
    100 & 63.89 & \textbf{27.78} & \textbf{38.72} & & \textbf{64.87} & 27.08 & 38.21 & $+0.70$ \\
    150 & \textbf{64.75} & \textbf{27.98} & \textbf{39.07} & & 64.66 & 26.40 & 37.49 & $+1.58$ \\
    \midrule
    \multicolumn{9}{l}{\emph{CIFAR-10}} \\
    50  & 84.48 & 50.47 & 63.19 & & \textbf{84.49} & \textbf{50.61} & \textbf{63.30} & $-0.14$ \\
    100 & \textbf{87.22} & 51.15 & \textbf{64.48} & & 85.24 & \textbf{51.52} & 64.22 & $-0.37$ \\
    150 & \textbf{87.37} & 51.33 & \textbf{64.67} & & 85.79 & \textbf{51.84} & 64.63 & $-0.51$ \\
    200 & \textbf{87.76} & \textbf{51.44} & \textbf{64.86} & & 86.13 & 51.33 & 64.33 & $+0.11$ \\
    \midrule
    \multicolumn{9}{l}{\emph{ARREST $+$ our radius, against CFA at the same mean radius}} \\
    100, $8.8/255$, C100 & 62.17 & \textbf{28.86} & \textbf{39.42} & & \textbf{64.08} & 27.53 & 38.51 & $+1.33$ \\
    100, $8.8/255$, C10  & 84.96 & 51.74 & 64.31 & & \textbf{85.93} & \textbf{51.76} & \textbf{64.60} & $-0.02$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
uses all four attacks in standard AutoAttack; FAB-T and Square leave the two-attack result unchanged
...
\paragraph{Repeated training.} Each reported row is an individual run. For the final CIFAR-100
configuration, seed $1$ gives $62.00/32.49/30.82/28.74$ clean/PGD-20/CW/AA, compared with
...
Unless explicitly ablated, CFA warm-starts from the natural teacher and trains with AdamW at
...
\begin{table}[!htbp]
  \centering \small \setlength{\tabcolsep}{4pt}
  \caption{Every baseline given our optimizer, schedule, natural warm start, training radius, weight
  averaging, and AWP proxy while retaining its own objective. CIFAR-100, ResNet-18.}
  \label{tab:fullmatch}
  \begin{tabular}{lrrrcrrrr}
    \toprule
...
seed-1 repeat of the final configuration, with $62.00\%$ clean accuracy and $28.74\%$ in its original
