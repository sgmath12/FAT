...
\begin{table}[!htbp]
  ...
  Each method keeps its own objective, optimizer, schedule and epoch count; the right block adds
  initialization at our natural teacher, weight averaging ($\kappa=0.999$ from $20\%$ of the run) and
  the AWP proxy ($\gamma=0.005$, warmup at $10\%$ of the run), with AWP evaluated on that method's own
  loss. Blank entries are runs still in progress. \dg ADR's published recipe already includes its own
  SAM-style AWP, so its left column is not stack-free; removing that term gives $58.22/20.93$
  (NRR $30.79$).}
...
The stack helps most of them, and by large margins where the objective was weakest: PGD-AT gains
$6.00$ NRR and ARD $6.45$, both from AutoAttack accuracy that rises by more than five points. It does
not help everywhere. HAT and ARREST lose $0.23$ and $0.17$ NRR, and both run short schedules where
there is little robust overfitting for weight averaging and AWP to undo --- ARREST finetunes for
twenty epochs. No stacked baseline reaches CFA, and the closest is ADR at its own recipe, $1.65$ NRR
behind. The comparison changes each baseline's recipe only by the components we add, so it separates
the stack from the objective without substituting our optimizer or schedule.
