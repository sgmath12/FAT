...
\Cref{sec:teacher_transfer} selects a teacher by training students. This appendix asks what can be read
off the frozen teacher instead, reports the predictions that failed as well as the one that held, and
records a measurement we retracted. Metrics are computed on $5000$ held-out images: clean accuracy, the
...
\lVert \delta \rVert_2$ for uniform $\delta$ at $8/255$. The students are the 50-epoch cells of
\Cref{tab:teacherladder_values}, identical apart from the teacher.
...
\begin{table}[!htbp]
  \centering \small \setlength{\tabcolsep}{4.5pt}
  \caption{Out-of-sample prediction: each line is fit on the nine trajectory checkpoints and applied to
  the three teachers of \Cref{tab:matchedteacher}, which were trained differently. Mean absolute error
  in accuracy points, and the error on each teacher.}
...
The ordering reverses between the two tables. The margin and the scatter ratio, strongest along the
trajectory, are the weakest predictors off it: the margin predicts $22.5$ and $20.2$ AutoAttack points
for the label-smoothing and mixup teachers, which give $25.72$ and $25.70$, and the scatter ratio errs
by $12.0$ points on the mixup teacher. Their trajectory correlations were the collinearity with
training time. Teacher accuracy predicts the robust axis best out of sample and the clean axis worst,
and feature sensitivity is the reverse. Neither the label-dependent gradient norm nor the margin is the
right quantity to read on a label-smoothing teacher in any case, since label smoothing changes the logit
scale both of them are computed from, which is a second reason to name the feature version in
\Cref{sec:teacher_transfer}.
...
do not match the network, and the three teachers of \Cref{tab:matchedteacher} use the same network as
the ladder.
