...
CIFAR-100 cell, on the full test set throughout.

\begin{table}[!htbp]
  \centering \small \setlength{\tabcolsep}{5pt}
  \caption{Gradient-masking checks on the shipped CIFAR-100 cell (clean accuracy $62.00$), all
  $10000$ test images. Each attack is run independently at $\epsilon = 8/255$, so the numbers are
  standalone rather than the cascade AutoAttack reports. The teacher-transfer row builds PGD-20 examples on the natural teacher
  ...
  \begin{tabular}{llr}
    ...
    APGD-CE            & white box, untargeted & \phantom{0}--- \\
    APGD-T             & white box, targeted   & \phantom{0}--- \\
    FAB-T              & white box, boundary   & \phantom{0}--- \\
    Square             & \textbf{black box}, 5000 queries & \phantom{0}--- \\
    \midrule
