...
\section{Proof and Scope of the Fixed-Anchor Decomposition}
\label{app:anchor_scope}
\begin{proof}[Proof of \Cref{prop:decomp}]
For any \(x'\in B(x,\epsilon)\), the triangle inequality gives
\[
\|\Phi_s(x')-\Phi_t(x)\|_2\le
\|\Phi_s(x')-\Phi_s(x)\|_2+\|\Phi_s(x)-\Phi_t(x)\|_2\le O+F,
\]
so \(L\le F+O\). Since \(x\in B(x,\epsilon)\), \(F\le L\). For any
\(x',x''\in B(x,\epsilon)\),
\[
\|\Phi_s(x')-\Phi_s(x'')\|_2\le
\|\Phi_s(x')-\Phi_t(x)\|_2+\|\Phi_s(x'')-\Phi_t(x)\|_2\le2L,
\]
so \(O\le2L\) and therefore \(F+O\le3L\).
\end{proof}
The result concerns exact maxima over the perturbation set; finite-step PGD
does not certify them. It applies to any fixed target in a normed space,
including logits, and does not imply that features are superior to logits or
that the bound alone guarantees robust classification.
...
We use a restricted version of the feature model of \citet{tsipras2018robustness}.
One feature $x_1$ agrees with the label with probability $p = 0.95$; here, we assume it cannot be
...
moved by an $\ell_\infty$ adversary. We take $\Phi_t = m(z)$ as a natural teacher with near-perfect
clean accuracy and poor robustness.
...
The fixed-target comparison in \Cref{sec:fragile} evaluates the teacher only
at the clean input. This choice is important because the natural teacher is
not locally robust: under \(\epsilon=8/255\), its feature rotates by
\(63.8^\circ\) and its norm grows by a factor of \(2.45\). The main-paper
control in \Cref{tab:teacherread} shows the consequence of replacing the
fixed clean target with the teacher's perturbed representation. Matching
\(\Phi_t(x_{\rm adv})\) makes an exact copy of the vulnerable teacher a
zero-loss solution and yields zero AutoAttack accuracy, whereas matching the
fixed clean target retains nonzero robustness. These diagnostics reinforce
that the teacher supplies a clean reference value rather than a robust
perturbed response.

\begin{table}[!htbp]
  \centering\small\setlength{\tabcolsep}{5pt}
  \caption{Effect of the teacher input used to define the regression target on
  CIFAR-100 with ResNet-18. Both variants use the same 50-epoch diagnostic
  setting; only the teacher input used by the target and inner maximization
  differs.}
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    \(\Phi_t(x)\) & 61.33 & 26.19 & \textbf{36.71} \\
    \(\Phi_t(x_{\rm adv})\) & 76.11 & 0.00 & 0.00 \\
    \bottomrule
  \end{tabular}
\end{table}
...
(\Cref{tab:recipe}). At the full training configuration, the AutoAttack gain
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lcrrrr}
    ...
    \textbf{Feature anchor, fixed head} & --- & --- & 62.72 & 25.88 & \textbf{36.64} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    50 epochs  & 75.81 & 1.100 & 64.28 & 0.955 & 24.38 \\
    100 epochs & 76.62 & 0.982 & 63.65 & 0.900 & 25.20 \\
    150 epochs & 77.52 & 0.887 & 62.93 & 0.875 & 25.78 \\
    200 epochs & 77.66 & 0.808 & 62.72 & 0.850 & 25.88 \\
    300 epochs & 78.32 & 0.712 & 62.24 & 0.829 & 25.80 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Baseline training-stack control.}
To test whether CFA's final performance can be explained by teacher
initialization, weight averaging, and AWP alone, we add the same components to
the baseline recipes. Most baselines improve, but none reaches CFA's NRR.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrcrrr}
    ...
    Consistency-AT & 57.53 & 20.01 & 29.69
                   && 60.74 & 25.38 & 35.80 \\
    LBGAT & 57.29 & 26.22 & 35.98
          && 55.75 & 25.57 & 35.06 \\
    ADR & 59.93 & 27.20 & \textbf{37.42}
        && 59.84 & 27.84 & \textbf{38.00} \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lr}
    ...
    \textbf{Natural teacher (ours)}  & 0.4763 \\
    Anchored student, $p=0$          & 0.0029 \\
    Anchored student, reported recipe & 0.0202 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \textbf{Feature anchor, head untouched}   & 62.72 & 28.69 & 27.80 & 25.88 & \textbf{36.64} \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrr}
    ...
    Raw features (squared $\ell_2$)            & 60.21 & 28.14 & 38.35 \\
    Both normalized (cosine)                   & 61.41 & 28.07 & \textbf{38.53} \\
    \midrule
    \multicolumn{4}{l}{\emph{(b) Base regime: no WA, no AWP, $\epsilon_{\mathrm{train}} = 8/255$}} \\
    Raw features (squared $\ell_2$)            & 62.40 & 24.34 & \textbf{35.02} \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    $8.8/255$\dg & 62.17 & 32.37 & 30.93 & 28.86 & \textbf{39.42} \\
    $10/255$\dg  & 59.99 & 32.82 & 30.53 & 28.77 & 38.89 \\
    \midrule
    ...
    $8.8/255$\dg & 84.96 & 52.98 & 53.94 & 51.74 & 64.31 \\
    $10/255$\dg  & 83.29 & 53.25 & 53.88 & 51.79 & 63.87 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \caption{AWP on top of weight averaging, across both datasets and both training radii. Every run
  uses the sensitivity-matched radius for 100 epochs; the AWP proxy uses $\gamma=0.005$ after a
  10-epoch warmup, and within each row AWP is the only added component. This is the right-hand edge of
  the factorial in \Cref{tab:ladder}, repeated on CIFAR-10 to check that the sign of the AWP effect
  ...
  \begin{tabular}{lrrrrrr}
    ...
    $8.8/255$ & 62.30 & 27.69 & \textbf{38.34} & 62.17 & 28.86 & \textbf{39.42} \\
    \midrule
    \multicolumn{7}{l}{\emph{CIFAR-10}} \\
    $8/255$   & 86.67 & 49.20 & 62.77 & 87.22 & 51.15 & \textbf{64.48} \\
    $8.8/255$ & 85.16 & 50.15 & \textbf{63.13} & 84.96 & 51.74 & 64.31 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrcrrr}
    ...
    \midrule
    PGD-AT, teacher initialization & 57.73 & 26.46 & \textbf{36.29} && 82.23 & 50.72 & \textbf{62.74} \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrcrrr}
    ...
    \quad $+$ AWP             & 57.48 & 28.05 & \textbf{37.70} & & 57.50 & 27.92 & \textbf{37.60} \\
...
reported configuration, the coefficient of variation of our sensitivity signal has median $0.56$,
...
The first-order derivation and the reported implementation use different sensitivity norms. \Cref{eqn:dual} gives the
$\ell_1$ norm and the reported implementation uses the $\ell_2$ gradient norm, which was fixed before this comparison and retained for reproducibility. The
two are not the same allocation.
Measured over training on the reported configuration, the $\ell_1$ signal's coefficient of variation
...
They nonetheless land close together. Training the reported recipe with
...
The difference is small but non-negligible relative to the observed seed variation. It is twice the seed-to-seed spread of \Cref{app:repro}, and it is the same
...
AutoAttack half does not. Thus the sensitivity norm is a genuine implementation choice: the reported $\ell_2$ variant performs slightly better in AA, while the clean-accuracy gain persists with the theoretically matched $\ell_1$ norm.

The permuted control in \Cref{tab:allocation} isolates the assignment signal from the radius distribution. It takes the multiset of weights the
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrr}
    \toprule
    Method & Clean & AA & Avg & NRR \\
    \midrule
    PGD-AT & 58.97 & 22.84 & 40.91 & 32.93 \\
    TRADES & 57.35 & 25.60 & 41.48 & 35.40 \\
    MART & 57.16 & 22.43 & 39.80 & 32.22 \\
    \midrule
    ARD & 59.52 & 23.05 & 41.29 & 33.25 \\
    RSLAD & 60.48 & 21.63 & 41.06 & 31.87 \\
    AdaAD & 61.66 & 25.62 & 43.64 & 36.20 \\
    LBGAT & 59.63 & 28.55 & 44.09 & 38.61 \\
    ARREST & 67.69 & 24.69 & 46.19 & 36.18 \\
    HAT & 62.81 & 26.91 & 44.86 & 37.68 \\
    ADR & 64.50 & 29.04 & 46.77 & 40.05 \\
    \midrule
    \textbf{CFA (ours)} & 65.04 & 30.14 & \textbf{47.59} & \textbf{41.19} \\
    \bottomrule
  \end{tabular}
\end{table}
...
We keep the inherited classifier fixed and omit the head-KD term, removing its temperature $\tau$
and weight $\beta$ from the objective. $p$ is not tuned. The training-radius sweep is reported in
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{llrrr}
    ...
    \midrule
    \textbf{CFA (ours)}            & 100 adv.  & 0.39 & 2.07 & 2.46 \\
...
cross-entropy at the same regime and about what PGD-AT costs once the teacher is included. The adversarial-distillation rows additionally rely on robust teachers with different training
requirements, so their total teacher--student cost is not directly comparable to CFA's natural
self-teacher setup.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrrr}
    ...
    200      & 77.90 & 4.542 & 4.129 & 63.02 & 26.16 & \textbf{36.97} \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{llrrrr}
    ...
    \midrule
    Anchor              & ours, teacher init   & 61.29 & 25.36 & \textbf{43.33} & \textbf{35.88} \\
...
Cross-entropy improves by $1.87$ NRR under its own schedule, and that stronger baseline is the one
carried into the comparisons in the main text. The anchor remains ahead under both tested
schedules. These comparisons support an objective-level contribution within the tested recipes, without establishing significance from
single runs or claiming exhaustive tuning of the alternatives.

The ladder in \Cref{tab:ladder} is reported as a factorial rather than as one ordered sequence,
because the order of addition is not identifiable from a single chain. Weight averaging and AWP are
crossed with each other on top of the same anchor and the same sensitivity-matched radius, and the
factorial is repeated at both training radii so that the pattern can be separated from the radius
choice.

\begin{table}[!htbp]
  \centering \small \setlength{\tabcolsep}{4.5pt}
  \caption{Weight averaging crossed with AWP on the ResNet-18 anchor (100 epochs, the
  sensitivity-matched radius with $p=1$ in every cell, evaluated at $\epsilon=8/255$). The three panels
  repeat the same $2\times2$ design at two training radii on CIFAR-100 and at the reported radius on
  CIFAR-10. The first row of each panel is the anchor with no stack component.}
  \label{tab:ladder}
  \begin{tabular}{llrrrr}
    \toprule
    WA & AWP & Clean & AA & Avg & NRR \\
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-100, $\epsilon_{\mathrm{train}}=8/255$}} \\
    --      & --      & 64.11 & 24.90 & 44.50 & 35.87 \\
    --      & \checkmark  & 65.01 & 27.62 & \textbf{46.32} & \textbf{38.77} \\
    \checkmark  & --      & 63.54 & 26.75 & 45.14 & 37.65 \\
    \checkmark  & \checkmark  & 63.89 & 27.78 & 45.84 & 38.72 \\
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-100, $\epsilon_{\mathrm{train}}=8.8/255$}} \\
    --      & --      & 63.09 & 25.74 & 44.41 & 36.56 \\
    --      & \checkmark  & 63.11 & 27.97 & \textbf{45.54} & 38.76 \\
    \checkmark  & --      & 62.30 & 27.69 & 44.99 & 38.34 \\
    \checkmark  & \checkmark  & 62.17 & 28.86 & 45.52 & \textbf{39.42} \\
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-10, $\epsilon_{\mathrm{train}}=8/255$}} \\
    --      & --      & 87.55 & 44.24 & 65.89 & 58.78 \\
    --      & \checkmark  & 87.76 & 50.44 & 69.10 & 64.06 \\
    \checkmark  & --      & 86.67 & 49.20 & 67.94 & 62.77 \\
    \checkmark  & \checkmark  & 87.22 & 51.15 & \textbf{69.19} & \textbf{64.48} \\
    \bottomrule
  \end{tabular}
\end{table}

Both components raise AA on their own, and their effects on AA are sub-additive in all three panels.
On CIFAR-100 at $8/255$, AWP alone is worth $+2.72$ AA and $+2.90$ NRR over the bare anchor and weight
averaging alone $+1.85$ AA and $+1.78$ NRR, but adding weight averaging on top of AWP returns only
$+0.16$ AA, and its clean cost of $1.12$ points leaves NRR flat ($-0.05$). At $8.8/255$ the first
component is worth $+2.20$ NRR (AWP) or $+1.78$ (weight averaging) and the second only $+0.66$ or
$+1.08$. CIFAR-10 shows the same shape with larger numbers: AWP alone gains $+6.20$ AA and weight
averaging alone $+4.96$, but weight averaging added on top of AWP is worth only $+0.71$ AA and AWP
added on top of weight averaging only $+1.95$.

The two components also differ on clean accuracy, consistently across datasets. Weight averaging
costs clean accuracy in all six of its cells, by between $0.54$ and $1.12$ points. AWP gains clean
accuracy in five of its six, by up to $0.90$ points, and the exception is a $0.13$-point loss at
$8.8/255$ with weight averaging already present. So the components are not interchangeable even where
their NRR contributions are similar: one buys robustness at essentially no clean cost, the other pays
for it.

Two readings are consistent with these panels --- much of what the stack contributes is available from
either component alone, and the two overlap in what they contribute --- and we do not try to separate
them further. We keep the full stack as the reported recipe because it gives the best AA in every
panel, the best NRR at $8.8/255$, and the best Avg and NRR on CIFAR-10; the one cell where it is not
first is CIFAR-100 at $8/255$, where AWP alone is ahead by $0.05$ NRR, smaller than the seed-to-seed
spread we measure at the reported configuration. Either choice leaves the comparisons in
\Cref{tab:main} unaffected, since every CFA row and every stack-matched baseline row carries the same
two components.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{llr}
    ...
    APGD-T             & white box, targeted   & 28.83 \\
    FAB-T              & white box, boundary   & 29.16 \\
    Square             & black box, 5000 queries & 34.82 \\
    \midrule
...
\paragraph{LBGAT} trains its natural branch jointly with the robust one, so unlike the distillation
...
Its CIFAR-10 cell was empty in an earlier version of this paper, and the reason was a transcription
error of ours rather than a property of their method. Their \texttt{adjust\_learning\_rate} is indexed
from one and runs before each epoch, so their first epoch trains at $0.02$ and the decays take effect
at epochs $76$ and $91$; we compared their epoch thresholds against a zero-based fractional epoch,
which moved the warmup to the second epoch and both decays one epoch late, leaving the first optimizer
step at the full $0.1$. Their target term is a mean over batch \emph{and} classes, so at equal
per-logit error a ten-class problem produces ten times the gradient of a hundred-class one, and that
first epoch is where it matters: CIFAR-100 absorbed the shift, while CIFAR-10 sat at chance from the
first step in both published variants. With the schedule corrected to match theirs at every epoch,
CIFAR-10 trains normally and is reported above. CIFAR-100 was re-measured under the same correction
and moved from $56.46/26.02$ to $57.29/26.22$, or $0.36$ NRR.

That correction postdates the Tiny-ImageNet and WideResNet LBGAT cells, which were measured before it
and are not re-run: a Tiny-ImageNet run costs $30$ hours here and the WideResNet AutoAttack evaluation
is launch-bound on this hardware. Neither collapsed, and the CIFAR-100 difference above bounds the
expected shift at well under half an NRR point for a hundred-class or two-hundred-class problem, but
they are one schedule revision behind the CIFAR ResNet-18 cells and we state that rather than imply a
single recipe across every LBGAT row.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{llrrrrrr}
    ...
    \midrule
    CIFAR-100 & CFA, head frozen (100 ep.)   & 62.17 & 36.91 & 32.37 & 30.93 & 28.86 & \textbf{39.42} \\
    ...
    \midrule
    CIFAR-10  & CFA, $\epsilon_{\mathrm{tr}} = 8.8/255$ (100 ep.) & 84.96 & 59.58 & 52.98 & 53.94 & 51.74 & 64.31 \\
              ...
              & CFA, $\epsilon_{\mathrm{tr}} = 8/255$  & 87.22 & 59.92 & 52.43 & 53.55 & 51.15 & \textbf{64.48} \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrl}
    ...
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-100, ResNet-18}} \\
    ...
    F\textsuperscript{2}AT    & 54.19 & 23.24 & 38.71 & 32.53 & \citet{f2at} \\
    Generalist++              & 62.97 & 23.96 & \textbf{43.47} & 34.71 & \citet{generalist} \\
    ADR (AT + ADR)            & 56.10 & 26.87 & 41.48 & 36.34 & ADR \\
    ADR (AT + WA + AWP + ADR) & 57.36 & 28.50 & 42.93 & \textbf{38.08} & ADR \\
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-10, ResNet-18}} \\
    ...
    IAD                       & 80.63 & 50.17 & 65.40 & 61.85 & CURE \\
    Consistency-AT + RPAT     & 84.12 & 48.98 & 66.55 & 61.91 & RPAT \\
    Generalist++              & 89.09 & 46.07 & 67.58 & 60.73 & \citet{generalist} \\
    ARREST                    & 86.63 & 46.14 & 66.39 & 60.21 & ARREST \\
    CURE                      & 86.76 & 49.69 & \textbf{68.23} & \textbf{63.19} & CURE \\
    ADR (AT + ADR)            & 82.41 & 50.38 & 66.40 & 62.53 & ADR \\
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-10, PreActResNet-18}} \\
    WA \citep{swa}            & 83.50 & 49.89 & 66.70 & 62.46 & RPAT \\
    AWP \citep{2020_awp}      & 81.11 & 50.09 & 65.60 & 61.93 & RPAT \\
    KD\,+\,SWA \citep{kdswa}   & 84.06 & 49.82 & \textbf{66.94} & 62.56 & RPAT \\
    TE \citep{te}             & 82.04 & 50.12 & 66.08 & 62.23 & RPAT \\
    ReBAT \citep{rebat}       & 82.09 & 50.72 & 66.41 & 62.70 & RPAT \\
    RPAT++ \citep{rpat}       & 82.63 & 51.00 & 66.82 & \textbf{63.07} & RPAT \\
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-100, PreActResNet-18}} \\
    WA \citep{swa}            & 57.26 & 25.83 & 41.55 & 35.60 & RPAT \\
    AWP \citep{2020_awp}      & 54.10 & 25.16 & 39.63 & 34.35 & RPAT \\
    KD\,+\,SWA \citep{kdswa}   & 57.17 & 25.66 & 41.42 & 35.42 & RPAT \\
    TE \citep{te}             & 56.41 & 25.84 & 41.13 & 35.44 & RPAT \\
    ReBAT \citep{rebat}       & 56.13 & 27.60 & 41.87 & 37.00 & RPAT \\
    RPAT++ \citep{rpat}       & 56.84 & 27.68 & \textbf{42.26} & \textbf{37.23} & RPAT \\
    \midrule
    \multicolumn{6}{l}{\emph{CIFAR-10, WideResNet-34-10}} \\
    ...
    KD\,+\,SWA \citep{kdswa}   & 87.45 & 53.59 & 70.52 & 66.46 & RPAT \\
    TE \citep{te}             & 85.97 & 52.88 & 69.43 & 65.48 & RPAT \\
    ReBAT \citep{rebat}       & 85.25 & 54.78 & 70.02 & 66.70 & RPAT \\
    ADR \citep{adr}           & 84.67 & 53.25 & 68.96 & 65.38 & RPAT \\
    CURE \citep{cure}         & 87.05 & 52.10 & 69.58 & 65.19 & RPAT \\
    RPAT++ \citep{rpat}       & 86.76 & 54.97 & \textbf{70.87} & \textbf{67.30} & RPAT \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrrrrl}
    ...
    Method & Clean & AA & Avg & NRR & Source \\
    \midrule
    \textbf{CFA (ours)}        & 55.16 & 20.54 & \textbf{37.85} & \textbf{29.93} & --- \\
    \midrule
    ...
    HAT                        & 52.60 & 18.14 & \textbf{35.37} & \textbf{26.98} & HAT \\
    ...
    Consistency-AT + RPAT      & 49.74 & 18.84 & \textbf{34.29} & \textbf{27.33} & RPAT \\
...
did not. Re-running the reported recipe with both closures agreeing gives $62.17/28.86$ against
...
All numbers reported for the reported recipe are post-correction. Two exceptions are stated where they
