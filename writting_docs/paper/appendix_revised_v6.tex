...
\begin{table}[!htbp]
  \centering \small \setlength{\tabcolsep}{4.5pt}
  \caption{Weight averaging crossed with AWP on the CIFAR-100 ResNet-18 anchor (100 epochs, the
  sensitivity-matched radius with $p=1$ in every cell, evaluated at $\epsilon=8/255$). The two panels
  repeat the same $2\times2$ design at the two training radii. The top-left cell of each panel is the
  anchor with no stack component.}
  \label{tab:ladder}
  \begin{tabular}{llrrrr}
    \toprule
    WA & AWP & Clean & AA & Avg & NRR \\
    \midrule
    \multicolumn{6}{l}{\emph{$\epsilon_{\mathrm{train}}=8/255$}} \\
    ...
    \midrule
    \multicolumn{6}{l}{\emph{$\epsilon_{\mathrm{train}}=8.8/255$}} \\
...
Both components raise AA on their own, and their effects on AA are sub-additive. At $8/255$ AWP alone
is worth $+2.72$ AA and $+2.90$ NRR over the bare anchor, while weight averaging alone is worth
$+1.85$ AA and $+1.78$ NRR; adding weight averaging on top of AWP then returns only $+0.16$ AA, and
its clean cost of $1.12$ points leaves NRR flat ($-0.05$). The same ordering holds at $8.8/255$,
where AWP alone gives $+2.20$ NRR and weight averaging alone $+1.78$, but the second component adds
only $+0.66$ and $+1.08$ NRR respectively. The two components also differ in sign on clean accuracy:
AWP never costs clean accuracy here and at $8/255$ gains $0.90$ points of it, whereas weight
averaging costs between $0.57$ and $1.12$ clean points in all four of its cells.

Two readings are consistent with this table --- much of what the stack contributes is available from
AWP alone, and the two components overlap in what they contribute --- and we do not try to separate
them further. We keep the full stack as the reported recipe because it gives the best AA in both
panels and the best NRR at $8.8/255$; at $8/255$ AWP alone is ahead of it by $0.05$ NRR, which is
smaller than the seed-to-seed spread we measure at the reported configuration. Either choice leaves
the comparisons in \Cref{tab:main} unaffected, since every CFA row and every stack-matched baseline
row carries the same two components.
