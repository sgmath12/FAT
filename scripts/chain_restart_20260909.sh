#!/usr/bin/env bash
# Restart after the machine went down (2026-09-09 07:55).  Only the cells with no completed result
# are listed; c10_ceatk_kdsym_t16 and c10_t50_anchor survived and are skipped.  Order is the one from
# 정리된생각.md: finish the two comparison blocks, then P0 -- the weak-teacher rungs, the warm-start
# control, and the seed repeats -- then the shipped-recipe logit cells and the standing queue.
set -u
cd "$(dirname "$0")/.."
PY=/home/seungju/miniforge3/envs/advTrain/bin/python
for entry in c10_ceatk_kdasym_t4 \
             c10_t50_logitmse c10_t50_kdsym c10_t50_kdasym \
