...
set -u
# Waits for the random-init CIFAR-100 run already in flight (pid 1627) instead of racing it; the
# previous chain script was edited while bash was still reading it, so it was replaced rather than
# amended.
cd "$(dirname "$0")/.."
...
echo "=== $(date '+%m-%d %H:%M') rpatpp_awp_c100_resnet18 finished ==="
