...
class WideResNet(nn.Module):
    ...
    @property
    def linear(self):
        # alias: train_feat_direction / span code reads .linear.weight uniformly, but WideResNet
        # names its classifier .fc. A property (not a registered submodule) keeps state_dict keys
        # as .fc, so WideResNet clean checkpoints still load strict=True.
        return self.fc

    def head_from_feat(self, out):
        # classifier on an already normalized+scaled feature (train_feat_direction head-KL split).
        return self.fc(out)
    ...
    def extract_feature(self, x, onlyfeature = False):
        out = self.conv1(x)
